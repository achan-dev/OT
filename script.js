...
const state = {
    ...
    tomatoMood: 'default',
    challengeClaims: JSON.parse(localStorage.getItem('challengeClaims')) || {},
    currentWeekOffset: 0,
...
const DataManager = {
    ...
    resetAllData() {
        ...
        document.getElementById('start-btn').style.background = '#FF6200';
...
const OvertimeManager = {
    startOvertime() {
        ...
        if (!state.currentStartTime) {
            ...
            startBtn.style.background = '#E55A00';
            ...
            startBtn.style.background = '#FF6200';
            ...
            typeChartCanvas.chart = new Chart(typeChartCanvas, {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        backgroundColor: ['#FF6200', '#A5D6A7', '#B0BEC5', '#FFCA28']
                    }]
                },
                options: {
                    responsive: true,
                    ...
                    plugins: {
                        ...
                        title: {
                            display: true,
                            text: state.currentTab === 'week' ? '本週加班類型' : '本月加班類型',
                            color: '#333',
                            font: {
                                size: 16,
                                family: "'Noto Sans TC', sans-serif"
            ...
            weeklyChartCanvas.chart = new Chart(weeklyChartCanvas, {
                ...
                options: {
                    ...
                    scales: {
                        y: {
                            beginAtZero: true,
                            title: { display: true, text: '時數（小時）', color: '#333' },
                            ticks: {
                        ...
                        x: { title: { display: true, text: '日期', color: '#333' } }
                    },
                    plugins: {
                        title: { display: true, text: `最近7天加班趨勢 (${state.currentWeekOffset === 0 ? '本' : state.currentWeekOffset > 0 ? `後${state.currentWeekOffset}` : `前${-state.currentWeekOffset}`} 週)`, color: '#333' }
                    }
                }
            });
        }
...
const SettingsManager = {
    saveSettings() {
        ...
        if (theme === 'blue') {
            ...
            document.body.style.background = '#F5F6FF';
        }
...
function shareApp() {
    const shareData = {
        title: '加班小番茄',
        text: '來試試這個有趣的加班記錄工具吧！幫你輕鬆追蹤加班時數，還能挑戰成就！',
        url: 'https://your-username.github.io/overtime-tomato/' // 替換為你的網站 URL
    };
    ...
    if (navigator.share) {
        ...
        navigator.clipboard.writeText(shareData.url)
            .then(() => {
                alert('網站連結已複製到剪貼簿！請分享給朋友吧！');
            })
            .catch(err => {
                console.error('複製失敗：', err);
                alert('無法複製連結，請手動複製：' + shareData.url);
            });
    }
}
